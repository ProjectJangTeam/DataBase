...
CREATE TABLE jang_alarm (
  ...
  ID     VARCHAR(50) NOT NULL COMMENT '아이디', -- 아이디
  CONT   MEDIUMTEXT  NOT NULL COMMENT '내용', -- 내용
  ALTIME DATETIME    NOT NULL COMMENT '알림시간', -- 알림시간
  RDTIME DATETIME    NULL     COMMENT '읽은시간' -- 읽은시간
)
COMMENT '알림';
...
CREATE TABLE jang_qna (
  ...
  ID      VARCHAR(50)  NOT NULL COMMENT '아이디', -- 아이디
  PBL     BOOLEAN      NOT NULL COMMENT '공개여부', -- 공개여부
...
CREATE TABLE jang_so_review (
  ...
  RECONO INTEGER      NOT NULL COMMENT '코스추천글번호', -- 코스추천글번호
  PLNAME VARCHAR(255) NOT NULL COMMENT '장소명', -- 장소명
  ADRS   VARCHAR(255) NULL     COMMENT '장소주소' -- 장소주소
)
COMMENT '장소후기';
...
CREATE TABLE jang_member (
  ...
  GENDER  CHAR(10)     NULL     COMMENT '성별', -- 성별
  SNS     VARCHAR(255) NULL     COMMENT 'SNS주소', -- SNS주소
  MBTI    VARCHAR(4)   NULL     COMMENT 'MBTI', -- MBTI
  STATUS  VARCHAR(10)  NOT NULL DEFAULT '활성' COMMENT '계정상태', -- 계정상태
  OUTDATE DATE         NULL     COMMENT '탈퇴일', -- 탈퇴일
  KAKAOID BIGINT       NULL     COMMENT '카카오ID' -- 카카오ID
)
COMMENT '회원';
...
CREATE TABLE jang_event_file (
  ITFNO INTEGER      NOT NULL COMMENT '이벤트첨부파일번호', -- 이벤트첨부파일번호
  PATH  VARCHAR(255) NULL     COMMENT '파일경로', -- 파일경로
  FNAME VARCHAR(50)  NULL     COMMENT '파일명', -- 파일명
  ITMNO INTEGER      NOT NULL COMMENT '이벤트상품번호' -- 이벤트상품번호
)
COMMENT '이벤트첨부파일';
...
ALTER TABLE jang_event_file
  MODIFY COLUMN ITFNO INTEGER NOT NULL AUTO_INCREMENT COMMENT '이벤트첨부파일번호';

-- 이벤트참여
CREATE TABLE jang_join (
  JOINO INTEGER     NOT NULL COMMENT '이벤트참여번호', -- 이벤트참여번호
  EVTNO INTEGER     NOT NULL COMMENT '이벤트번호', -- 이벤트번호
  ID    VARCHAR(50) NOT NULL COMMENT '아이디', -- 아이디
  WIN   BOOLEAN     NOT NULL COMMENT '당첨여부' -- 당첨여부
)
COMMENT '이벤트참여';
...
ALTER TABLE jang_join
  MODIFY COLUMN JOINO INTEGER NOT NULL AUTO_INCREMENT COMMENT '이벤트참여번호';

-- 이벤트상품
CREATE TABLE jang_item (
  ITMNO INTEGER      NOT NULL COMMENT '이벤트상품번호', -- 이벤트상품번호
  NAME  VARCHAR(255) NOT NULL COMMENT '상품명', -- 상품명
  CNT   INTEGER      NOT NULL COMMENT '상품재고', -- 상품재고
  EVTNO INTEGER      NOT NULL COMMENT '이벤트번호', -- 이벤트번호
  RANK  INTEGER      NOT NULL COMMENT '지급순위' -- 지급순위
)
COMMENT '이벤트상품';
...
ALTER TABLE jang_item
  MODIFY COLUMN ITMNO INTEGER NOT NULL AUTO_INCREMENT COMMENT '이벤트상품번호';
...
CREATE TABLE jang_event (
  ...
  TITLE  VARCHAR(255) NOT NULL COMMENT '제목', -- 제목
  CONT   MEDIUMTEXT   NOT NULL COMMENT '내용', -- 내용
  STDATE DATETIME     NOT NULL COMMENT '시작일', -- 시작일
  EDDATE DATETIME     NOT NULL COMMENT '종료일', -- 종료일
  PBL    BOOLEAN      NOT NULL COMMENT '공개여부', -- 공개여부
...
ALTER TABLE jang_event
  MODIFY COLUMN EVTNO INTEGER NOT NULL AUTO_INCREMENT COMMENT '이벤트번호';
